...
| T-103 | P3 | Paste 2 or 3 real Udemy review quotes if you want them surfaced on the landing | Course stats are now accurate (see T-014), but no individual review quote is rendered on the site. Udemy ships reviews only to logged-in or paginated modal contexts, not the course landing HTML, so scraping text without an authenticated session returns nothing useful. If you want quoted testimonials, export a few top reviews from the instructor dashboard and paste them into a Slack/chat message. Otherwise we leave the current paraphrased testimonial that cites the aggregate numbers. |
...
| T-107 | P1 | Create the Cal.com event `jorgesalgadomiranda/30min` | The primary CTA on the site points at `https://cal.com/jorgesalgadomiranda/30min` and Cal.com returns 404 for both that URL and the bare `/jorgesalgadomiranda` profile. Anyone clicking Book a call today lands on a Cal.com 404. Fix: sign up for Cal.com with the username `jorgesalgadomiranda` and create a public 30-minute event with slug `30min`. `MANUAL_SETUP.md` section 5 has the full checklist. If you prefer a different Cal.com username or slug, swap both `href` occurrences in `index.html`. |
